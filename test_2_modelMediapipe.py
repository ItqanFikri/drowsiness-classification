...
model_1 = load_model('./Itqan/2_model/IndRNNAug5400_1&2.h5', custom_objects={'IndRNN': IndRNN}, compile=False)  # Pastikan model sudah dilatih dan disimpan
model_2 = load_model('./Itqan/2_model/IndRNNAug5400_1&3.h5', custom_objects={'IndRNN': IndRNN}, compile=False)  # Pastikan model sudah dilatih dan disimpan
model_3 = load_model('./Itqan/2_model/IndRNNAug5400_2&3.h5', custom_objects={'IndRNN': IndRNN}, compile=False)  # Pastikan model sudah dilatih dan disimpan

data = pd.read_csv('./Itqan/data_dummy_bukaan.csv')
...
prediction_1 = model_1.predict(input_sequence)
prediction_2 = model_2.predict(input_sequence)
prediction_3 = model_3.predict(input_sequence)

# Menyusun kelas akhir berdasarkan prediksi model
final_predictions = []

for i in range(len(input_sequence)):  # Misalnya jika input_sequence adalah batch data
    pred_1 = np.argmax(prediction_1[i])  # Hasil dari model 1
    pred_2 = np.argmax(prediction_2[i])  # Hasil dari model 2
    pred_3 = np.argmax(prediction_3[i])  # Hasil dari model 3
    
    # Menyesuaikan hasil prediksi
    if pred_1 == 0:
        label_1 = 1  # Kelas 1 dari model 1
    else:
        label_1 = 2  # Kelas 2 dari model 1

    if pred_2 == 0:
        label_2 = 1  # Kelas 1 dari model 2
    else:
        label_2 = 3  # Kelas 3 dari model 2

    if pred_3 == 0:
        label_3 = 2  # Kelas 2 dari model 3
    else:
        label_3 = 3  # Kelas 3 dari model 3

    # Gabungkan hasil prediksi
    # Majority Voting: pilih kelas yang paling banyak muncul
    # Gabungkan ke dalam array untuk voting
    labels = [label_1, label_2, label_3]
    final_class = np.bincount(labels).argmax()  # Pilih kelas yang paling banyak muncul
    
    final_predictions.append(final_class)
    
print(prediction_1)  # Hasil dari model 1
print(prediction_2)  # Hasil dari model 2
print(prediction_3)  # Hasil dari model 3   
print(f'Final predictions for the input sequence: {final_predictions}')
...
# Mengakses kamera
# cap = cv2.VideoCapture('F:/Dataset TA/Video/38/10.mp4')
...
while True:
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            if frame_count % 1800 == 0:
                ...
                for i in range(len(input_sequence)):  # Untuk setiap data dalam batch input
                    # Probabilitas dari tiap model
                    prob_1 = prediction_1[i]  # Model 1 memprediksi kelas 1 dan 2
                    prob_2 = prediction_2[i]  # Model 2 memprediksi kelas 1 dan 3
                    prob_3 = prediction_3[i]  # Model 3 memprediksi kelas 2 dan 3

                    # Prediksi argmax untuk masing-masing model
                    pred_1 = np.argmax(prob_1)
                    pred_2 = np.argmax(prob_2)
                    pred_3 = np.argmax(prob_3)

                    # Menyesuaikan prediksi ke kelas global
                    label_1 = 1 if pred_1 == 0 else 2  # Model 1: kelas 1 atau 2
                    label_2 = 1 if pred_2 == 0 else 3  # Model 2: kelas 1 atau 3
                    label_3 = 2 if pred_3 == 0 else 3  # Model 3: kelas 2 atau 3

                    # Majority Voting
                    labels = [label_1, label_2, label_3]
                    bincount = np.bincount(labels, minlength=4)  # Hitung jumlah tiap kelas (kelas 1-3)
                    max_votes = np.max(bincount)

                    if np.sum(bincount == max_votes) == 1:
                        # Jika ada mayoritas (tidak imbang), ambil kelas dengan votes terbanyak
                        final_class = np.argmax(bincount)
                    else:
                        # Voting seimbang, gunakan probabilitas gabungan
                        # Probabilitas gabungan untuk setiap kelas
                        prob_class_1 = (prob_1[0] + prob_2[0]) / 2  # Kelas 1: dari Model 1 & 2
                        prob_class_2 = (prob_1[1] + prob_3[0]) / 2  # Kelas 2: dari Model 1 & 3
                        prob_class_3 = (prob_2[1] + prob_3[1]) / 2  # Kelas 3: dari Model 2 & 3

                        combined_probs = [0, prob_class_1, prob_class_2, prob_class_3]  # Kelas global
                        final_class = np.argmax(combined_probs)  # Pilih kelas dengan probabilitas gabungan tertinggi

                    # Simpan prediksi akhir
                    final_predictions.append(final_class)
    
                    
                # Output hasil prediksi akhir
                print("Final predictions for the input sequence:", final_predictions)
                
            # Menampilkan hasil EAR, MAR, dan prediksi pada frame
            cv2.putText(frame, f'EAR: {ear:.2f}', (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            cv2.putText(frame, f'MAR: {mar:.2f}', (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            ...
            cv2.putText(frame, f'Prediction Model 1: {prediction_1}', (30, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            cv2.putText(frame, f'Prediction Model 2: {prediction_2}', (30, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            cv2.putText(frame, f'Prediction Model 3: {prediction_3}', (30, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            cv2.putText(frame, f'Final Prediction: {final_predictions}', (30, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            ...
            with open(output_file, "a") as file:
                file.write(f"{frame_count}///{ear:.2f}///{mar:.2f}///{final_predictions}///{fps:.2f}\n")
    ...
    cv2.putText(frame, f'FPS: {fps:.2f}', (30, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
